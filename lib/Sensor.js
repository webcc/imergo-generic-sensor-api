...
module.exports = class Sensor extends EventTarget
{
    ...
    initDefaults()
    {
        ...
        this._onchange = event => console.log(event.reading);
        this._onstatechange = event => console.log(this.state);
    ...
    set reading(value)
    {
        ...
        {
            this.dispatchEvent(new SensorReadingEvent("change", { reading: this.reading }));
        }
    }
    ...
    start()
    {
        ...
        {
            if(this.state === SensorState.ACTIVATED || this.state === SensorState.ERRORED)
            {
    ...
    stop()
    {
        return new Promise((resolve, reject) =>
        {
            ...
            if(this.state === SensorState.IDLE || this.state === SensorState.ERRORED)
            {
                reject(new InvalidStateError())
            }
            else
